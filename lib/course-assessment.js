...
// This is an explicit main-history alias, not a question-id fallback. The
// first adaptive knowledge-check release persisted this exact main question
// under the chapter-level unit key before the module-level key was enforced.
const LEGACY_MAIN_REGRADE_CONTEXTS = new Map([
  [
    "GH-05-post-q4|V14-C2|V14-C2-post|post",
    {
      chapterId: "V14-C2",
      unitId: "GH-05-post",
      phase: "post",
      gradingFingerprint: "68ebd928e112b6c7915e3c71eb71d175c18ceaf6386b582206890849b4b00b8e"
    }
  ]
]);
...
function gradingQuestionFingerprint(question = {}) {
  const signature = {
    id: String(question.id || ""),
    type: String(question.type || ""),
    questionText: String(question.question || question.prompt || question.title || question.text || ""),
    referenceAnswer: String(question.referenceAnswer || question.answerText || question.analysis || ""),
    rubric: String(question.rubric || question.commentPrompt || ""),
    points: Math.max(0, Number(question.points || 0))
  };
  return nodeCrypto.createHash("sha256").update(JSON.stringify(signature)).digest("hex");
}

function legacyMainRegradeEntry(index, query, entry) {
  ...
  const alias = LEGACY_MAIN_REGRADE_CONTEXTS.get(
    `${questionId}|${chapterId}|${unitId}|${phase}`
  );
  if (!alias || !entry) return null;
  if (
    entry.chapterId !== alias.chapterId
    || entry.unitId !== alias.unitId
    || entry.phase !== alias.phase
    || gradingQuestionFingerprint(entry.question) !== alias.gradingFingerprint
  ) {
    return null;
  }
  return entry;
...
function assessmentEntry(index, query = {}, options = {}) {
  const questionId = String(query.questionId || query.question_id || "").trim();
  ...
  const unitId = String(query.unitId || query.unit_id || "").trim();
  const phase = String(query.phase || "").trim();
  const exactContext = (!chapterId || entry.chapterId === chapterId)
    && (!unitId || entry.unitId === unitId)
    && (!phase || entry.phase === phase);
  if (!exactContext) {
    return options.allowLegacyMainRegradeContext
      ? legacyMainRegradeEntry(index, query, entry)
      : null;
  }
  return entry;
}
...
function authoritativeGradingQuestions(index, rows = [], options = {}) {
  return (rows || []).flatMap((row) => {
    ...
    const entry = assessmentEntry(index, row, {
      allowLegacyMainRegradeContext: options.allowLegacyMainRegradeContext === true
    });
